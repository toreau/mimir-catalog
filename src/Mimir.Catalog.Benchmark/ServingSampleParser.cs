...
public static class ServingSampleParser
{
    ...
    public static IReadOnlyList<ServingTimedSample> Parse(byte[] bytes)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            if (line.Length == 0)
            {
                // A single trailing empty element from a final LF is allowed; nothing else.
                if (isLast && i == lines.Length - 1) continue;
                throw new ServingSampleParseException($"blank record at line {i + 1}");
            }
    ...
    private static ServingTimedSample ParseLine(string json, int lineNumber)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                switch (name)
                {
                    ...
                    case "wall_seconds":
                        ...
                        break;
                    case "correctness_status": status = Str(reader, name, lineNumber); break;
            ...
            if (operation.Length == 0) throw new ServingSampleParseException($"line {lineNumber}: 'operation' must be non-empty");
            if (stratum.Length == 0) throw new ServingSampleParseException($"line {lineNumber}: 'stratum' must be non-empty");
